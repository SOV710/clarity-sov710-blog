...
::link-card
---
title: A deeper investigation of inotify
icon: https://www.google.com/s2/favicons?domain=lwn.net&sz=64
link: https://lwn.net/Articles/605128
...
icon: https://www.google.com/s2/favicons?domain=lwn.net&sz=64
link: https://lwn.net/Articles/718802/
class: gradient-card active
---
::
...
::link-card
---
title: The 2018 Linux Storage, Filesystem, and Memory-Management Summit
icon: https://www.google.com/s2/favicons?domain=lwn.net&sz=64
link: https://lwn.net/Articles/lsfmm2018/
class: gradient-card active
---
::
...
现在只能在文件被 `open` for write 时得到通知；如果文件已打开，再次写入或 mmap 写入不会触发通知。他希望有一种方式：能在文件第一次被真正写入前冻结、刷盘，并产生通知 → 记录到 change journal。理想效果类似“pre-write one-shot mark”：只在首次修改时触发，之后解除冻结。提议可能需要新的 VFS hook 或安全钩子，但进入 page-fault 路径的复杂度和锁问题让内核开发者担心。

::quote
缺少子树级过滤
::

希望内核能支持“把某目录标记为子树根”，只报告该子树内的事件。macOS 有类似功能；Btrfs 的子卷处理也和这个有共通点。不过社区（如 Jan Kara）担心强制隔离子树（禁止 rename 进出、hard link 跨越）语义怪异，不好让用户理解。

::link-card
---
title: Lazy file reflink
icon: https://www.google.com/s2/favicons?domain=lwn.net&sz=64
link: https://lwn.net/Articles/789038/
class: gradient-card active
---
::

2025 LSFMM: Goldstein 继续先前的话题，提出了两种实现思路，一种是和前面一样做写入前后各发一次事件进行 hook，第二种是提供一种新的系统调用，使得用户需要一致性时，写入操作是通过 SRCU 挂起的，直到写入操作全部完成再统一写入。

::quote
#icon
😓
#default
内核社区认可需求场景，但担心复杂度和性能；目前尚未达成共识。
::
...
icon: https://www.google.com/s2/favicons?domain=lwn.net&sz=64
link: https://lwn.net/Articles/1017947/
class: gradient-card active
---
::
